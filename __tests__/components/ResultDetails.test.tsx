...
describe("ResultDetails", () => {
  ...
  it("should show a loading indicator", () => {
    mockFetch(mockMovie);
    render(<ResultDetails id={123} type="movie" />);
  ...
  it("should render movie details and availability correctly", async () => {
    mockFetch(mockMovie);
    render(<ResultDetails id={mockMovie.id} type="movie" />);
    ...
    await waitFor(async () => {
      ...
      expect(otherLink).toHaveAttribute(
        "href",
        mockMovie.availability.otherCountries[0].watchLink
      );
    });
  });
  ...
  it("should render tv show details and availability correctly", async () => {
    mockFetch(mockTv);
    render(<ResultDetails id={mockTv.id} type="tv" />);
  ...
  it('should show "No streaming availability found" when availability is empty', async () => {
    ...
    render(<ResultDetails id={movieWithoutAvailability.id} type="movie" />);

    await waitFor(() => {
      expect(
        screen.getByText("No streaming availability found")
      ).toBeInTheDocument();
    });
  });
});
